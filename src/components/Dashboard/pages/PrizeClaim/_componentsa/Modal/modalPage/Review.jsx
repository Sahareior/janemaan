...
console.log('claimselected', selectedClaim)
const {data:vouchers} = useGetAllVoucherQuery()

const getVoucherCodebyId =(id)=>{
const isExists = vouchers?.find(items => items.id === id)
    ...
    <div>
      <div className="space-y-8 mt-5 text-sm sm:text-base md:text-[17px] px-2 pb-10 sm:px-4">
        ...
        <h3 className="text-white flex gap-3">
          Voucher Code: <span className="text-[#97BECA]">{getVoucherCodebyId(selectedClaim?.voucher)}</span>
        </h3>
        ...
        <div className="space-y-4">
          ...
          <div className="w-full rounded-lg bg-[#1F2937] overflow-hidden">
            <img
