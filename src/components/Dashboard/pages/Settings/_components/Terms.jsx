...
import { useGetPolicyQuery, useUpdatePolicyMutation } from "../../../../../redux/slices/apiSlice";
...
const Terms = ( ) => {
  ...
  const {data:Terms,isLoading,refetch} = useGetPolicyQuery()
...
const handleLogContent = async () => {
  if (quillRef.current) {
    ...
    try {
      const res = await updatePolicy({ content: text }).unwrap();
